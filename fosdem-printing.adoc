...
[.notes]
--
* There is the core mismatch: semantical markup meets the rigid world of print.
* Take a simple listing... (go through very briefly just to give filling)
** Stop on orientation -- it is already not about semantical markup.
...
[.notes]
--
* Despite we live with Paged Media CSS for almost 15 years, HTML -- still poor browser support, usually used via PDF. HTML for printing is intermediary format. There is a great open source solutions to convert it to PDF -- Weasy Print and a number of a very good non opensource solutions. But still HTML is an intermediate format
...
* PDF <- via TeX (sometimes via TeXML and then TeX)
...
[.notes]
--
* Documentation of UniDoc Publisher is created with Asciidoctor and is published with WeasyPrint, XSL-FO and Open Document toolchains. Looks pretty the same. But just as a proof of concept

* Native libraries examples: ReportLab in Python, Prawn in Ruby, PDFBox in Java
* Some FOP processors Apache FOP, XEP
* If we have time we'll compare, there is a slide at the end
...
[.notes]
--
Two speleologist meet in a narrow adit.
I'm from a dead end.
Great, I'm too!
...
--
[%step]
. Asciidoctor parses markup into AST
...
[.notes]
--
get_basic_style -- is some external function, defined as a helper
--
...
HTML is quite semantical, why shouldn't we use it?
...
[.notes]
--
Stress, that this is typical AST transforming code
--
...
[.notes]
--
Rearranging looks like slim (or XSLT) templates. Not so slim as Slim, but definitely slimer than XSLT
--
...
[.notes]
--
Just say that this is about inserting horizontal lines before notes and preventing slide from breaking.
...
[.notes]
--
Extending AST is quite simple. But it is rarely used. This is artificial example, easiear solution would be to add paragraph directly
--
...
But it is already Ivan Ponomarev's talk
--
...
* Still more coincidence, they met LibreOffice
...
//Bonus slides: rendening technology comparison
...
|===
|Technology |Strengths |Weaknesses
|image:images/tex.png[width="80px"]
