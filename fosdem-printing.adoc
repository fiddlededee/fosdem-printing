...
[[a601]]
== With no custom transformations and styling

image::images/no-transformations.png[width=58%]

[.notes]
--
* [REF] CSS styles are not automatically applied: in many cases it is not possible and sometimes simply unnecessary. This results in ugly image sizes, no code highlighting, no special styling for notes
* [REF] No Epigraph. I just forgot to implement quoted element in the standard Asciidoctor adapter
* [REF] Unnecessary Header on a first page. The reason is that First page style is not automatically applied to the first paragraph

[TIP]
====
UniDoc Publisher is a library for building converters. It doesn't aim to produce a good-looking document out of the box, it  aims at reducing effort required to design your own converter

Still, the result is more or less readable
====
--
...
[.notes]
--
If we have Asciidoctor with some Printing converter platform on top and our Custom converter on top of the top, and some LibreOffice on top of the top of the top we should be sure, that minor changes won't break anything
...
* Gradle + Kotlin stack is just a coincidence, where clear syntax, wide ecosystem and CI accidentally met
